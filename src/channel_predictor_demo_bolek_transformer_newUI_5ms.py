# final_demo.py
# 最终版: 融合Transformer, 使用真实SP/NP, 支持长序列滑动窗口可视化
# v10: [修正] 将Transformer的错误标记也移动到图表底部(y=0)，使其更加直观。
...
class PerformanceReplayEngine:
    def __init__(self, h_real, sp_np_real, model, scaler, device):
        ...
        self.results = {'reactive': {'throughputs': [], 'errors': []}, 'transformer': {'throughputs': [], 'errors': []}, 'real_snr': []}
    ...
    def run_simulation(self):
        ...
        for t in range(start_t, self.h_real.shape[0] - HORIZON):
            ...
            end_index = t - 1
            start_index = end_index - (LOOKBACK - 1) * TS_STEP
            history_indices = np.arange(start_index, end_index + 1, TS_STEP)
            history_complex = self.h_real[history_indices]
            ...
            snr_transformer_input = self._sp_np_to_snr_db(pred_signal_power, predicted_noise_power) #####
...
def run_visualization(sim_results):
    ...
    fig.suptitle('Performance Replay Engine: Reactive vs. Predictive', fontsize=20)
    ax1.set_title('Real-time Throughput'); ax1.set_ylabel('Data Rate (Mbps)')
    line_reactive, = ax1.plot([], [], 'o-', label=f'Reactive System (t+{HORIZON} delay)', color='orange', lw=2, zorder=1)
    line_transformer, = ax1.plot([], [], '*-', label=f'Transformer System (H={HORIZON} Predict)', color='blue', lw=2, zorder=2)
    max_tp = max(max(sim_results['reactive']['throughputs']), max(sim_results['transformer']['throughputs'])) if sim_results['transformer']['throughputs'] else 1
    ax1.set_ylim(0, max_tp * 1.15 if max_tp > 0 else 10)
    
    text_template = 'System: {}\nTotal Data: {:.1f} Mb\nTotal Errors: {}\nData Rate (1s avg): {:.2f} Mbps'
    
    text_reactive = ax1.text(0.98, 0.95, '', transform=ax1.transAxes, fontsize=14, verticalalignment='top', horizontalalignment='right', bbox=dict(boxstyle='round', facecolor='orange', alpha=0.4), zorder=10)
    text_transformer = ax1.text(0.98, 0.68, '', transform=ax1.transAxes, fontsize=14, verticalalignment='top', horizontalalignment='right', bbox=dict(boxstyle='round', facecolor='blue', alpha=0.4), zorder=10)
    error_marker_r, = ax1.plot([], [], 'x', color='red', markersize=10, label='Reactive Error', alpha=0.8, mew=2.5, zorder=5)
    error_marker_t, = ax1.plot([], [], 'x', color='darkviolet', markersize=10, label='Transformer Error', alpha=0.8, mew=2.5, zorder=5)
    ax1.legend(loc='upper left')
    ax2.set_title('Ground Truth Channel Quality'); ax2.set_ylabel('SNR (dB)'); ax2.set_xlabel('Time Step (5ms)')
    ...
    ax2.set_xlim(0, VIEW_WINDOW_SIZE); ax2.set_ylim(min(real_snr_history) - 5, max(real_snr_history) + 5); ax2.legend(loc='upper left')

    def update(frame):
        steps_per_second = int(1 / TIME_STEP_INTERVAL_S)
        window_start = max(0, frame + 1 - steps_per_second)
        reactive_window = sim_results['reactive']['throughputs'][window_start:frame+1]
        avg_rate_r = np.mean(reactive_window) if len(reactive_window) > 0 else 0
        transformer_window = sim_results['transformer']['throughputs'][window_start:frame+1]
        avg_rate_t = np.mean(transformer_window) if len(transformer_window) > 0 else 0
        ...
        text_reactive.set_text(text_template.format(f"Reactive (t+{HORIZON})", total_data_r, total_err_r, avg_rate_r))
        ...
        text_transformer.set_text(text_template.format("Transformer", total_data_t, total_err_t, avg_rate_t))
        ...
        for collection in ax1.collections: collection.remove()
        ax1.fill_between(x_data_window, reactive_throughputs_window, transformer_throughputs_window, where=np.array(transformer_throughputs_window) > np.array(reactive_throughputs_window), facecolor='green', alpha=0.3, interpolate=True, zorder=0)
        ax1.fill_between(x_data_window, reactive_throughputs_window, transformer_throughputs_window, where=np.array(reactive_throughputs_window) >= np.array(transformer_throughputs_window), facecolor='red', alpha=0.3, interpolate=True, zorder=0)
        ...
        return line_reactive, line_transformer, line_real_snr, time_marker, text_reactive, text_transformer, error_marker_r, error_marker_t
    
    ani = animation.FuncAnimation(fig, update, frames=len(real_snr_history), blit=False, interval=1, repeat=False)
...
if __name__ == "__main__":
    ...
    DATA_PATH_H = 'data/my_H_real_data_bolek_for_5ms_40m_10mps_for_demo.mat'
    DATA_PATH_SP_NP = 'data/my_SP_NP_real_data_bolek_for_5ms_40m_10mps_for_demo.npy'
    ...
    try:
        ...
        print(f"错误: 未找到与当前配置匹配的模型或Scaler文件({e})。请先运行训练脚本。")
        exit()
    except RuntimeError as e:
        print(f"错误: 加载模型失败，可能是模型结构不匹配: {e}")
        print(f"请确认当前脚本中的 LOOKBACK ({LOOKBACK}), HORIZON ({HORIZON}) 等参数与您训练时所用的完全一致。")
        exit()

...
def epoch_loop(model: nn.Module, train_loader: DataLoader, test_loader: DataLoader, 
    ...
    for epoch in range(epochs):
        ...
        if test_loss < best_loss:
            best_loss = test_loss
            epochs_no_improve = 0
            model_save_path = os.path.join(results_dir, f'{model_name}_{horizon}_best_model_magnitude_bolek.pth')
            scaler_save_path = os.path.join(results_dir, f'{model_name}_{horizon}_scaler_magnitude_bolek.gz')
...
def main():
    ...
    epochs = 500
    batch_size = 512
    learning_rates = {'GRU': 0.000025, 'LSTM': 0.0001, 'CNN': 0.0001, 'MLP': 0.0001, 'Transformer': 0.000001}
    ...
    results_file = os.path.join(results_dir, f'performance_results_magnitude_lookback{lookback}_bolek.csv')

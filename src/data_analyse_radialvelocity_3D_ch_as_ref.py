...
def plot_3d_flight_path(df, mission_name):
    ...
    z_range = z.max() - z.min()
    # 确保没有范围为0的情况，避免除零错误
    ax.set_box_aspect([x_range if x_range > 0 else 1, 
...
if __name__ == '__main__':
    ...
    # 确保在脚本同级目录下有一个名为 'data' 的文件夹
    data_dir = os.path.join(os.getcwd(), 'data')
    channel_log_path = os.path.join(data_dir, 'ch_est_pavel_40m_10mps_from_big_rec_autopilot.txt')
    gps_log_path = os.path.join(data_dir, 'pavel_40m_10mps.csv')
    ...
    try:
        ...
        merged_df = pd.merge_asof(
            left=channel_df_full.sort_index(),  # 信道数据作为主表
            right=gps_df_full.sort_index(),      # GPS数据作为待匹配表
            on='timestamp',
            direction='nearest',               # 寻找最近的时间点
            tolerance=pd.Timedelta('1s')       # 容忍1秒的时间差
        )
        # 清理掉未能成功匹配到GPS数据的行
        merged_df.dropna(subset=['gps.lat', 'gps.lon', 'altitudeAMSL'], inplace=True)
        
        if merged_df.empty:
              raise ValueError("数据同步失败：在信道数据的时间范围内，未能匹配到任何有效的GPS数据点。")
        
        # === FIX: Re-establish the DatetimeIndex after merging ===
        ...
        print(f"\n错误: {e}")
        print("请检查 'data' 文件夹是否存在，以及所需的数据文件是否在其中。")

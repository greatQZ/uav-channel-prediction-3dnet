...
def generate_samples_from_block_s2s(block_2d: np.ndarray, lookback: int, horizon: int, step: int) -> Tuple[np.ndarray, np.ndarray]:
    ...
    # 总长度需求是 lookback 的输入 + horizon 的输出
    # 这里为了简化，假设 step 主要用于构建输入序列
    required_len = (lookback - 1) * step + horizon 
    num_samples = num_timesteps - required_len
    ...
    for i in range(num_samples):
        ...
        # 输出/目标序列的索引 (关键修改)
        # 目标应该从输入结束后的下一个点开始，连续取 horizon 个点
        # 假设预测的步长为1
        indices_out = np.arange(input_end_idx + 1, input_end_idx + 1 + horizon)
        y.append(block_2d[indices_out])
        
    # y 的形状现在是 (num_samples, horizon, num_features)
    return np.array(X), np.array(y)
...
def epoch_loop(model: nn.Module, train_loader: DataLoader, test_loader: DataLoader, 
    ...
    for epoch in range(epochs):
        ...
        if test_loss < best_loss:
            ...
            # 保存所有模型
            model_save_path = os.path.join(results_dir, f'{model_name}_{horizon}_best_model_magnitude.pth')
            scaler_save_path = os.path.join(results_dir, f'{model_name}_{horizon}_scaler_magnitude.gz')
...
def plot_predictions(true_vals: np.ndarray, preds: np.ndarray, model_name: str, 
    ...
    # 对Naive预测进行偏移处理，确保正确对齐
    # Naive预测应该使用前一个时间步的值，所以需要向右偏移一个时间步
...
def main():
    ...
    epochs = 500
    batch_size = 512
    learning_rates = {'GRU': 0.000025, 'LSTM': 0.000025, 'CNN': 0.000025, 'MLP': 0.000025, 'Transformer': 0.000025}
    ...
    results_file = os.path.join(results_dir, f'performance_results_magnitude_lookback{lookback}.csv')
    ...
    for horizon in horizons:
        ...
        # 计算Naive预测（使用前一个时间步的值）
        # 对于M2O数据，Naive预测应该是输入序列的最后一个时间步
        ...
        for model_name in models_to_run:
            ...
            if model_name == 'Transformer':
                ...
                model = TimeSeriesTransformer(
                    input_size=NUM_ACTIVE_SUBCARRIERS, dim_val=1024, 
                ...
                if model_name == 'GRU':
                    model = GRUNet_Seq2Seq(
                        input_dim=NUM_ACTIVE_SUBCARRIERS, hidden_dim=1024, 
            ...
            #print("First 5 naive predictions:", naive_predictions[:5, subcarrier_idx])
            
            # 计算并打印相关系数
            #correlation = np.corrcoef(true_vals[:100, subcarrier_idx], naive_predictions[:100, subcarrier_idx])[0, 1]

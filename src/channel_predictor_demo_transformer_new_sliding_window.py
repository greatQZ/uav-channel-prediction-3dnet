# final_demo.py
# 最终版: 融合Transformer(H=1), 使用真实SP/NP, 支持长序列滑动窗口可视化
...
class PerformanceReplayEngine:
    ...
    def run_simulation(self):
        ...
        for t in range(start_t, self.h_real.shape[0]):
            ...
            predicted_noise_power = self.sp_np_real[t - HORIZON, 1]
...
def run_visualization(sim_results):
    ...
    line_transformer, = ax1.plot([], [], '*-', label=f'Transformer System (Horizon={HORIZON} Predict)', color='blue', lw=2, zorder=2)
    ax1.legend()
    ...
    ax1.set_ylim(-1, max_tp * 1.15 if max_tp > 0 else 10)
    ...
    text_transformer = ax1.text(0.98, 0.78, '', transform=ax1.transAxes, fontsize=14, verticalalignment='top', horizontalalignment='right', bbox=dict(boxstyle='round', facecolor='blue', alpha=0.4), zorder=10)
    ...
    def update(frame):
        ...
        line_reactive.set_data(x_data_window, sim_results['reactive']['throughputs'][start_index:frame+1])
        line_transformer.set_data(x_data_window, sim_results['transformer']['throughputs'][start_index:frame+1])
        ...
        if frame >= VIEW_WINDOW_SIZE:
            ax1.set_xlim(frame - VIEW_WINDOW_SIZE, frame + 10)
        return line_reactive, line_transformer, line_real_snr, time_marker, text_reactive, text_transformer
    ani = animation.FuncAnimation(fig, update, frames=len(real_snr_history), blit=True, interval=10, repeat=False, cache_frame_data=False)
...
if __name__ == "__main__":
    ...
    DATA_PATH_H = 'data/my_H_real_data_heli_for_final_demo.mat'
    DATA_PATH_SP_NP = 'data/my_SP_NP_real_data_heli_for_final_demo.npy'
    ...
    try:
        ...
        model = TimeSeriesTransformer(
            input_size=NUM_SUBCARRIERS, dim_val=1024, n_encoder_layers=2, n_decoder_layers=2, n_heads=8, 
            dropout_encoder=0.1, dropout_decoder=0.1, dropout_pos_enc=0.1, dim_feedforward_encoder=1024, 

# live_demo_final_version.py
# 反应式系统的延迟与HORIZON参数动态关联，实现最公平对比
...
def predict_transformer_autoregressive(model, x_norm_tensor, horizon, device):
    ...
    with torch.no_grad():
        ...
        for _ in range(horizon):
            tgt_mask = torch.triu(torch.ones(decoder_input.size(1), decoder_input.size(1)) * float('-inf'), diagonal=1).to(device)
...
OH_PER_RB = 0
LOOKBACK = 5
HORIZON = 1  # <--- 您可以调整这个值, Reactive的延迟会自动匹配
NUM_SUBCARRIERS = 576
...
class PerformanceReplayEngine:
    def __init__(self, h_real, snr_real_db, model, scaler, device, noise_power_dbm):
        self.h_real = h_real
        self.snr_real_db = snr_real_db
        self.model = model
        self.scaler = scaler
        self.device = device
    ...
    def _predicted_mag_to_snr_db(self, pred_mag_vector):
        pred_power = np.mean(pred_mag_vector**2)
        if pred_power < 1e-15: return -100.0
        return 10 * np.log10(pred_power / self.noise_power_linear)
    ...
    def run_simulation(self):
        print("正在基于真实的SNR测量值运行仿真...")
        start_t = LOOKBACK + HORIZON - 1
        for t in range(start_t, self.h_real.shape[0]):
            snr_real_current = self.snr_real_db[t]
            self.results['real_snr'].append(snr_real_current)
            
            # --- 1. Reactive System ---
            # <<< --- 关键改动：延迟与HORIZON动态关联 --- >>>
            snr_reactive_input = self.snr_real_db[t - HORIZON]
            ...
            if self._is_tx_successful(cqi_r, snr_real_current):
                ...
                self.results['reactive']['throughputs'].append(tbs / 1000.0); self.results['reactive']['errors'].append(0)
            else:
                self.results['reactive']['throughputs'].append(0); self.results['reactive']['errors'].append(1)
            
            # --- 2. Transformer System ---
            history_start = t - (LOOKBACK + HORIZON - 1); history_end = t - (HORIZON - 1)
            ...
            pred_step_norm = prediction_norm_tensor[:, 0, :].cpu().numpy()
            pred_step_mag = self.scaler.inverse_transform(pred_step_norm).squeeze()
            snr_transformer_input = self._predicted_mag_to_snr_db(pred_step_mag)
            ...
            if self._is_tx_successful(cqi_t, snr_real_current):
                ...
                self.results['transformer']['throughputs'].append(tbs / 1000.0); self.results['transformer']['errors'].append(0)
            else:
...
def run_visualization(sim_results):
    ...
    fig.suptitle('Performance Replay Engine: Reactive vs. Transformer', fontsize=20)
    ax1.set_title('Real-time Throughput'); ax1.set_ylabel('Data Rate (Mbps)'); ax1.grid(True, linestyle='--', alpha=0.6)
    
    # <<< --- 关键改动：图例与HORIZON动态关联 --- >>>
    line_reactive, = ax1.plot([], [], 'o-', label=f'Reactive System (t-{HORIZON} delay)', color='orange', lw=2)
    line_transformer, = ax1.plot([], [], '*-', label=f'Transformer System (H={HORIZON} Predict)', color='blue', lw=2)
    ax1.legend()
    ...
    ax1.set_ylim(-5, max_tp * 1.15 if max_tp > 0 else 10)
    ...
    text_transformer = ax1.text(0.02, 0.78, '', transform=ax1.transAxes, fontsize=14, verticalalignment='top', bbox=dict(boxstyle='round', facecolor='blue', alpha=0.3))
    ...
    def update(frame):
        ...
        line_transformer.set_data(x_data, sim_results['transformer']['throughputs'][:frame+1])
        line_real_snr.set_data(x_data, real_snr_history[:frame+1])
        time_marker.set_xdata([frame, frame])
        total_data_r = np.sum(sim_results['reactive']['throughputs'][:frame+1]) * 0.001
        total_err_r = np.sum(sim_results['reactive']['errors'][:frame+1])
        text_reactive.set_text(text_template.format(f"Reactive (t-{HORIZON})", total_data_r, total_err_r))
        total_data_t = np.sum(sim_results['transformer']['throughputs'][:frame+1]) * 0.001
        total_err_t = np.sum(sim_results['transformer']['errors'][:frame+1])
        text_transformer.set_text(text_template.format("Transformer", total_data_t, total_err_t))
        return line_reactive, line_transformer, line_real_snr, time_marker, text_reactive, text_transformer
...
if __name__ == "__main__":
    ...
    DATA_PATH_H = 'data/heli_flight_H_real_data.mat'
    DATA_PATH_SNR = 'data/heli_flight_SNR_real_data.npy'
    ...
    try:
        ...
        model = TimeSeriesTransformer(input_size=NUM_SUBCARRIERS, dim_val=1024, n_encoder_layers=2, n_decoder_layers=2, n_heads=8, dropout_encoder=0.1, dropout_decoder=0.1, dropout_pos_enc=0.1, dim_feedforward_encoder=1024, dim_feedforward_decoder=1024, max_seq_len=LOOKBACK+HORIZON, dec_seq_len=LOOKBACK, out_seq_len=HORIZON)
    ...
    try:
        ...
        snr_real_db = np.load(DATA_PATH_SNR)
        print(f"成功从 {DATA_PATH_SNR} 加载了 {len(snr_real_db)} 个时间步的SNR数据（不含padding）。")
        ...
        if len(h_real_clean) != len(snr_real_db):
            print(f"警告：清洗后的H数据长度({len(h_real_clean)})与SNR数据长度({len(snr_real_db)})不匹配！")
            min_len = min(len(h_real_clean), len(snr_real_db))
            h_real_clean = h_real_clean[:min_len]
            snr_real_db = snr_real_db[:min_len]
            print(f"已将数据统一截断为较短的长度: {min_len}")
            
    except FileNotFoundError as e:
        print(f"错误: 未找到数据文件: {e}。程序将退出。"); exit()
    
    if h_real_clean.shape[0] > NUM_TIMESTEPS:
        print(f"数据过长，截取前 {NUM_TIMESTEPS} 个时间步进行仿真。")
        h_real_clean = h_real_clean[:NUM_TIMESTEPS, :]; snr_real_db = snr_real_db[:NUM_TIMESTEPS]

    engine = PerformanceReplayEngine(h_real_clean, snr_real_db, model, scaler, device, NOISE_POWER_DBM)

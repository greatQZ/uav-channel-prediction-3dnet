...
def plot_power_fluctuation_vs_radial_velocity_bins(df, mission_name):
    """
    【新功能】通过分箱和箱形图，深入分析径向速度对信道功率波动的影响。
    """
    print(f"\n" + "="*50)
    print(f"开始为任务 '{mission_name}' 生成功率波动 vs 径向速度分箱图...")

    # 确保用于分析的列没有空值
    analysis_df = df[['radial_velocity_abs', 'power_change_rate']].dropna()

    if len(analysis_df) < 50: # 需要足够的数据点才有意义
        print("!!! 警告: 数据点不足，无法生成有意义的分箱分析图。")
        return

    # 创建径向速度的分箱
    # 我们将速度范围分成5个等宽的箱子
    try:
        analysis_df['velocity_bin'] = pd.cut(analysis_df['radial_velocity_abs'], bins=5, precision=2)
    except Exception as e:
        print(f"!!! 警告: 创建速度分箱失败: {e}。可能是速度变化范围过小。")
        return

    plt.figure(figsize=(14, 8))
    sns.boxplot(x='velocity_bin', y='power_change_rate', data=analysis_df, palette='vlag')
    
    plt.title(f'Power Fluctuation vs. Radial Velocity Bins for {mission_name}', fontsize=16)
    plt.xlabel('Absolute Radial Velocity Bins (m/s)', fontsize=12)
    plt.ylabel('Power Fluctuation (dB change)', fontsize=12)
...
if __name__ == '__main__':
    ...
    try:
        ...
        # 【新功能调用】
        plot_power_fluctuation_vs_radial_velocity_bins(merged_df, mission_name)

...
改进说明:
1. 修复了Transformer模型评估和预测逻辑
2. 消除了数据泄露风险
3. 统一了评估指标计算
4. 修复了绘图函数假设所有模型都是M2O的问题
5. 添加了完整的随机种子设置
6. 改进了设备管理
7. 优化了内存使用
8. 保存所有模型而不仅仅是Transformer
9. 添加了学习率过小时的提前停止
10. 减少了代码冗余
"""
...
def train(model: nn.Module, train_loader: DataLoader, criterion: nn.Module, 
    ...
    for x, y, _ in train_loader:
        ...
        # 统一计算损失：比较预测序列的最后一个点与目标序列的第horizon个点
...
def evaluate(model: nn.Module, test_loader: DataLoader, device: torch.device, 
    ...
    with torch.no_grad():
        for i, (x, y, y_clean) in enumerate(test_loader):
            ...
            # 统一评估标准：比较预测序列的最后一个点与目标序列的第horizon个点
...
def main():
    ...
    for horizon in horizons:
        ...
        # 使用生成器函数减少内存使用
        def block_generator(blocks, mode='m2o'):
            for block in blocks:
                if mode == 'm2o':
        ...
        class BlockDataset(IterableDataset):
            def __init__(self, blocks, mode='m2o'):
                self.blocks = blocks
                self.mode = mode
...
#            if model_name == 'Naive Outdated':
                # 修改Naive预测的计算方式
                # 对于horizon=1，Naive预测应该是输入序列的最后一个时间步
#                start_time = time.time()
#                predictions = test_x_m2o[:, -1, :]
...
#                print("First 5 input last steps:", test_x_m2o[:5, -1, 0])
#                print("First 5 target values:", test_y_m2o[:5, 0])
    
                # 如果对齐有问题，我们可以尝试使用不同的方式
                # 例如，对于horizon=1，Naive预测应该是前一个时间步的值
                # predictions = np.roll(test_y_m2o, 1, axis=0)
                # predictions[0] = test_x_m2o[0, -1, :]  # 第一个预测使用输入序列的最后一个时间步
    

#                print("Naive prediction method: Using last step of input sequence")
#               print(f"Shape of predictions: {predictions.shape}")
#                print(f"Shape of targets: {test_y_m2o.shape}")
    
#                # 检查前几个样本的对齐情况
#                for i in range(min(5, len(test_x_m2o))):
#                    print(f"Sample {i}:")
#                    print(f"  Input sequence: {test_x_m2o[i, :, 0]}")
#                    print(f"  Last step of input: {test_x_m2o[i, -1, 0]}")
#                    print(f"  Target value: {test_y_m2o[i, 0]}")
#                    if i > 0:
#                        print(f"  Previous target value: {test_y_m2o[i-1, 0]}")
#    
#                end_time = time.time()
#                total_time = end_time - start_time
...
#    
#                # 添加更多调试信息
#                print(f"First 5 predictions: {predictions[:5, 0]}")
#                print(f"First 5 targets: {targets[:5, 0]}")
#                print(f"MSE: {mse:.6f}")
...
###############################################################################################################
            ...
            subcarrier_idx = 0 # 选择要绘制的子载波索引
            
            # 在绘图之前添加调试输出
            print(f"True values shape: {true_vals.shape}")
            print(f"Predictions shape: {preds.shape}")
            print(f"Naive predictions shape: {naive_preds.shape}")

            # 打印前几个值以验证对齐
            print("First 5 true values:", true_vals[:5, subcarrier_idx])
            print("First 5 predictions:", preds[:5, subcarrier_idx])
            print("First 5 naive predictions:", naive_preds[:5, subcarrier_idx])
            # 计算并打印相关系数
            correlation = np.corrcoef(true_vals[:100, subcarrier_idx], naive_preds[:100, subcarrier_idx])[0, 1]
            print(f"True values and naive predictions correlation: {correlation:.4f}")
            
################################################################################################################
            plot_predictions(true_vals, preds, model_name, naive_preds, num_timesteps=150, subcarrier_idx=subcarrier_idx)

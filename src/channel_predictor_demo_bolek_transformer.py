...
class PerformanceReplayEngine:
    ...
    def run_simulation(self):
        ...
        for t in range(start_t, self.h_real.shape[0] - HORIZON):
            ...
            snr_real_current = self._sp_np_to_snr_db(sp_real_current, np_real_current)-10 # 添加10dB偏移以匹配训练数据
            self.results['real_snr'].append(snr_real_current)

            # --- 1. Reactive System (基于 t 时刻的测量值，模拟 H 步的延迟) ---
            sp_reactive, np_reactive = self.sp_np_real[t]
            snr_reactive_input = self._sp_np_to_snr_db(sp_reactive, np_reactive)-10 # 同样添加10dB偏移
            ...
            # --- 2. Transformer System (用 t-LOOKBACK 到 t-1 的数据预测 t+HORIZON) ---
            ...
            # --- 关键Bug修复：之前取[0]，现在取[HORIZON-1]以获得真正的远期预测 ---
            ...
            predicted_noise_power = self.sp_np_real[t, 1] # 使用t时刻的真实噪声作为噪声预测
            snr_transformer_input = self._sp_np_to_snr_db(pred_signal_power, predicted_noise_power)-10 # 同样添加10dB偏移
...
if __name__ == "__main__":
    # --- 修改2：更新文件路径以匹配训练脚本生成的文件名格式 ---
    MODEL_PATH = f'results/Transformer_lookback{LOOKBACK}_horizon{HORIZON}_best_model_magnitude_bolek.pth'
    SCALER_PATH = f'results/Transformer_lookback{LOOKBACK}_horizon{HORIZON}_scaler_magnitude_bolek.gz'
    # 注意: 演示数据文件路径已根据您的旧脚本更新
    DATA_PATH_H = 'data/my_H_real_data_bolek_for_demo.mat'
    DATA_PATH_SP_NP = 'data/my_SP_NP_real_data_bolek_for_demo.npy'
    ...
    try:
        ...
        # --- 修改3：修正模型初始化参数，使其与训练时完全一致 ---
        model = TimeSeriesTransformer(
            input_size=NUM_SUBCARRIERS, 
            dim_val=1024, # 使用您验证有效的简化模型参数
            n_encoder_layers=2, 
            n_decoder_layers=2, 
            n_heads=32, 
            dropout_encoder=0.2, # 使用您验证有效的正则化参数
            dropout_decoder=0.2, 
            dropout_pos_enc=0.1, 
            dim_feedforward_encoder=1024, # 使用您验证有效的简化模型参数

...
# 最终确认版: 融合Transformer模型，使用真实的H, SP, NP数据，并动态关联延迟与Horizon
...
class PerformanceReplayEngine:
    ...
    def run_simulation(self):
        ...
        for t in range(start_t, self.h_real.shape[0]):
            ...
            predicted_noise_power = self.sp_np_real[t - HORIZON, 1] # 使用t-H时刻的真实噪声作为噪声预测
...
def run_visualization(sim_results):
    ...
    fig.suptitle('Performance Replay Engine: Reactive vs. Predictive(Transformer)', fontsize=20)
    ...
    text_reactive = ax1.text(0.78, 0.95, '', transform=ax1.transAxes, fontsize=14, verticalalignment='top', bbox=dict(boxstyle='round', facecolor='orange', alpha=0.3),zorder=10)
    text_transformer = ax1.text(0.78, 0.78, '', transform=ax1.transAxes, fontsize=14, verticalalignment='top', bbox=dict(boxstyle='round', facecolor='blue', alpha=0.3), zorder=10)
...
if __name__ == "__main__":
    ...
    DATA_PATH_H = 'data/my_H_real_data_heli_for_verifying.mat'
    DATA_PATH_SP_NP = 'data/my_SP_NP_real_data_heli_for_verifying.npy'
    ...
    if h_real_clean.shape[0] > NUM_TIMESTEPS:
        ...
        h_real_clean = h_real_clean[:NUM_TIMESTEPS, :]; sp_np_real_data = sp_np_real_data[:NUM_TIMESTEPS]

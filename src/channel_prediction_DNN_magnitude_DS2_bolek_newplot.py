...
def plot_all_models_predictions(true_vals: np.ndarray, naive_preds: np.ndarray, 
    ...
    """
    Plots true values, naive predictions, and all deep learning model predictions on one chart.
    Interactively asks user whether to save the plot.
    """
    plt.figure(figsize=(15, 7))
    
    # Determine the minimum length to avoid index errors
    min_length = len(true_vals)
    if len(naive_preds) < min_length:
        min_length = len(naive_preds)
    for preds in model_preds_dict.values():
        if len(preds) < min_length:
            min_length = len(preds)
    
    # Restrict to num_timesteps for visualization
    ...
    plt.plot(time_steps, true_vals[:plot_len, subcarrier_idx], 
             label='True Magnitude', color='black', linewidth=2.5, alpha=0.8)
    
    # Plot Naive Prediction
    plt.plot(time_steps, naive_preds[:plot_len, subcarrier_idx], 
             label='Naive Prediction', color='green', linestyle=':', linewidth=2, alpha=0.8)
    ...
    print(f"\n[System] 正在显示 Horizon={horizon} 的对比图预览窗口...")
    plt.show(block=False) # Show window but don't block execution completely
    plt.pause(0.5)        # Render the window content
    
    # Pause script execution and ask for user input
    user_input = input(f" >>> [询问] 是否保存 Horizon={horizon} 的对比图片? (y/n): ").strip().lower()
    
    if user_input == 'y':
        save_filename = f'comparison_horizon{horizon}_sub{subcarrier_idx}_all_models.png'
        save_path = os.path.join(save_dir, save_filename)
        plt.savefig(save_path, dpi=300)
        print(f" >>> [保存] 图片已保存至: {save_path}")
    else:
        print(" >>> [跳过] 图片未保存。")

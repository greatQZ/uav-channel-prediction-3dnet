...
def parse_gps_log(file_path):
    ...
    【已修改】增加了 on_bad_lines='skip' 来处理格式错误的行。
    """
    if not os.path.exists(file_path):
    ...
    # =======================================================================
    #  *** 代码修改处 ***
    #  添加 on_bad_lines='skip' 参数。
    #  Pandas 2.2 之后推荐使用 on_bad_lines='skip' 或 on_bad_lines='warn'
    #  它会跳过那些列数不匹配的行，从而避免程序因数据文件中的个别错误行而中断。
    # =======================================================================
    try:
...
def plot_full_profile(gps_df_full, channel_df_full, merged_df, mission_name):
    ...
    # =======================================================================
    #  *** 核心修改处：手动固化时间戳 ***
    #  这是一种与环境无关的强制方法，确保Matplotlib无法进行任何时区转换。
    #  1. .strftime(...) 将带时区的索引（已经是正确的柏林时间）转换为字符串。
    #  2. pd.to_datetime(...) 再将该字符串解析回一个“朴素的”时间对象。
    # =======================================================================
    ...
    # 由于我们已经手动处理了时间，定位器和格式化程序不再需要任何时区信息
...
if __name__ == '__main__':
    ...
    try:
        ...
        if not gps_df_full.empty:
            ...
            gs_alt = gps_df_full['altitudeAMSL'].iloc[0] + 5.0 # 高度加5米
            print("\n" + "="*50)
            print("根据GPS起点估算基站坐标:")
        ...
        merged_df = pd.merge_asof(
            left=channel_df_full.sort_index(),
            right=gps_df_full.sort_index(),
            on='timestamp',
            direction='nearest',
        ...
        # =======================================================================
        #  *** 代码修改处 ***
        #  恢复这一行，将'timestamp'列重新设置为DataFrame的索引
        # =======================================================================
